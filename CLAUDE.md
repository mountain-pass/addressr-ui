...
Problem tickets live under `docs/problems/` and follow the `wr-itil:manage-problem` skill's filename-suffix convention (mirrors the decision-file convention in DECISION-MANAGEMENT.md):

- `NNN-kebab-title.open.md` — reported, under investigation
- `NNN-kebab-title.known-error.md` — root cause confirmed, fix path clear, fix not yet released
- `NNN-kebab-title.verifying.md` — fix released, awaiting user verification (positive-path observed; user explicitly confirms before close)
- `NNN-kebab-title.parked.md` — blocked on upstream or suspended by user decision
- `NNN-kebab-title.closed.md` — fix verified by user

Status is also duplicated in the YAML frontmatter `status:` field for greppability. Transitions use `git mv` (rename), then a frontmatter edit to update the duplicated status, then the relevant content additions (a `## Fix Released` section on close-to-verifying, a `## Closed` section on verifying-to-closed, a `## Parked` section on parking). Per the skill's P057 staging trap, re-stage explicitly after the Edit since `git mv` alone only stages the rename.

The `docs/problems/README.md` cache (WSJF rankings, Verification Queue, Parked, Resolved) is regenerated by `/wr-itil:manage-problem review`.
